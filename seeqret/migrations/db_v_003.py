"""Migration 003 -- Slack exchange support.
   ...
   Mirrors jseeqret's v3 schema so the two tools can share a vault:

     - users gains slack_handle, slack_key_fingerprint and
       slack_verified_at columns
     - new ``kv`` table holds Fernet-encrypted Slack tokens and
...
def init_db_v_003(vault_dir):
    """Apply schema migration 003 to the vault database.

       - Add columns slack_handle, slack_key_fingerprint and
         slack_verified_at to the users table.
       - Create a new ``kv`` table for Fernet-encrypted configuration
         (Slack user token, channel id, last_seen_ts and friends).
    """
