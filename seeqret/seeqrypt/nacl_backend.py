...
def asymetric_encrypt_string(string: str, sender_private_key: PrivateKey, recipient_public_key: PublicKey) -> str:
    box = Box(private_key=sender_private_key, public_key=recipient_public_key)
    return encoding.Base64Encoder.encode(box.encrypt(string.encode('utf-8'))).decode('ascii')


def asymetric_decrypt_string(string: str, receiver_private_key: PrivateKey, sender_public_key: PublicKey) -> str:
    box = Box(private_key=receiver_private_key, public_key=sender_public_key)
    return box.decrypt(encoding.Base64Encoder.decode(string.encode('ascii'))).decode('ascii')
