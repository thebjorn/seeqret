...
The 4-byte prefix holds the real payload length. The receiver uses it
to strip the random tail before passing the ciphertext on.
...
def pad_to_bucket(payload: bytes, bucket: int = DEFAULT_BUCKET) -> bytes:
    ...
    The final blob is exactly ``N * bucket`` bytes long, including the
